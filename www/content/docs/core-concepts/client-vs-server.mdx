...
Each call returns an independent instance bound to `appEvents`; choose its lifetime in application code. A long-running process may own one server instance because user context is supplied per event, while an isolated request or worker may create and shut down its own.
...
await Promise.all([
  serverAnalytics.track('first_event', firstInput),
  serverAnalytics.track('second_event', secondInput)
]);
```

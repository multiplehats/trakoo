...
Create custom providers to integrate trakoo with any analytics service.
...
Server-side providers can also expose `shutdown()`. `ServerAnalytics.shutdown()` calls it when the provider has that method.
...
See the examples above for detailed custom provider patterns.

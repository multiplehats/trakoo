---
title: Introduction
description: Type-safe, provider-agnostic analytics for TypeScript applications
---

import { Step, Steps } from 'fumadocs-ui/components/steps';

**trackle** is a highly typed, zero-dependency, provider-agnostic analytics library for TypeScript applications. Track events with full type safety across client and server environments.

## Why trackle?

<Cards>
  <Card title="Fully Type-Safe" icon="target">
    Define your events once, get autocomplete and type checking everywhere. No more typos or wrong properties.
  </Card>
  <Card title="Provider Agnostic" icon="plug">
    Use any analytics service—PostHog, Bento, Pirsch, or build your own. Switch providers without changing your code.
  </Card>
  <Card title="Universal API" icon="globe">
    Same API works on client and server. Write once, track everywhere.
  </Card>
  <Card title="Zero Dependencies" icon="package">
    Core library has zero dependencies. Providers are optional and tree-shakeable.
  </Card>
</Cards>

## Quick Example

```typescript
import { createClientAnalytics } from 'trackle/client';
import { PostHogClientProvider } from 'trackle/providers/client';

// Define your events with full type safety
const appEvents = {
  userSignedUp: {
    name: 'user_signed_up',
    category: 'user',
    properties: {} as {
      email: string;
      plan: 'free' | 'pro' | 'enterprise';
    }
  }
} as const;

// Initialize with your providers
const analytics = createClientAnalytics<typeof appEvents>({
  providers: [
    new PostHogClientProvider({ token: 'your-key' })
  ]
});

// Track events with autocomplete and type checking
analytics.track('user_signed_up', {
  email: 'user@example.com',
  plan: 'pro'  // ✅ TypeScript knows this must be 'free' | 'pro' | 'enterprise'
});
```

## Get Started in 3 Steps

<Steps>
<Step>

### Install the Library

```bash
pnpm install trackle
```

</Step>

<Step>

### Choose Your Providers

Install the SDKs for your analytics services:

```bash
# PostHog
pnpm install posthog-js posthog-node

# Bento
pnpm install @bentonow/bento-node-sdk

# Pirsch
pnpm install pirsch-sdk
```

</Step>

<Step>

### Start Tracking

Define your events and start tracking:

```typescript
import { createClientAnalytics } from 'trackle/client';

const analytics = createClientAnalytics({
  providers: [/* your providers */]
});

analytics.track('button_clicked', { buttonId: 'cta' });
```

</Step>
</Steps>

## Key Features

### 🎯 Type-Safe Events

Define your events with TypeScript and get compile-time validation:

```typescript
// ❌ This will error - wrong property
analytics.track('user_signed_up', {
  wrongProperty: 'value'
});

// ❌ This will error - wrong event name
analytics.track('wrong_event', {});

// ✅ This works - fully typed
analytics.track('user_signed_up', {
  email: 'user@example.com',
  plan: 'pro'
});
```

### 🔌 Plugin Architecture

Add multiple providers and they all receive events:

```typescript
const analytics = createClientAnalytics({
  providers: [
    new PostHogClientProvider({ token: 'xxx' }),
    new BentoClientProvider({ siteUuid: 'xxx' }),
    new PirschClientProvider({ identificationCode: 'xxx' })
  ]
});

// One call, all providers receive it
analytics.track('purchase', { amount: 99.99 });
```

### 🌐 Universal API

Same API works on both client and server:

```typescript
// Client-side
import { createClientAnalytics } from 'trackle/client';

// Server-side
import { createServerAnalytics } from 'trackle/server';

// Both support the same methods
analytics.track();
analytics.identify();
analytics.pageView();
```

### 👤 Automatic User Context

Identify users once, context flows to all events:

```typescript
// Identify the user
analytics.identify('user-123', {
  email: 'user@example.com',
  plan: 'pro'
});

// All subsequent events include user context automatically
analytics.track('feature_used', {
  feature: 'export'
});
// Providers receive: { userId: 'user-123', email: 'user@example.com', traits: {...} }
```

## What's Next?

<Cards>
  <Card title="Installation" href="/docs/installation" icon="download">
    Install the library and get set up
  </Card>
  <Card title="Your First Event" href="/docs/your-first-event" icon="rocket">
    Track your first event in under 2 minutes
  </Card>
  <Card title="Core Concepts" href="/docs/core-concepts/events" icon="book">
    Deep dive into events, providers, and type safety
  </Card>
  <Card title="Framework Guides" href="/docs/guides/nextjs" icon="code">
    Integration guides for Next.js, SvelteKit, and more
  </Card>
</Cards>

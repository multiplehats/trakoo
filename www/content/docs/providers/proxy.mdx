...
The Proxy ships with trakoo, so it has no extra package of its own. Install the provider package and server SDK for each server provider you forward to—for example, `@trakoo/posthog` with `posthog-node` for PostHog, or `@trakoo/emitkit` with `@emitkit/js` for [EmitKit](/docs/providers/emitkit). [Pirsch](/docs/providers/pirsch) ships with trakoo and needs no additional package.
...
This is in-memory, at-least-once retry behavior rather than exactly-once or
durable delivery. A batch can reach your endpoint more than once when the
server accepts a request but its response is lost. Queued events can still be
lost if the page or process terminates before the browser accepts a request.
Make downstream event handling tolerant of duplicates when that matters.

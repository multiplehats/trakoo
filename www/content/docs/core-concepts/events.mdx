...
Split large definition objects by domain, then combine them in the single registry passed to factories:

```typescript title="lib/events/index.ts"
import { defineEvents } from 'trakoo';
import { productEventDefinitions } from './product';
import { userEventDefinitions } from './user';
...
Keep definition fragments as plain objects and call `defineEvents()` once after merging. This guarantees duplicate wire names are checked across the full application registry.

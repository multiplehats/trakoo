...
trakoo provides separate APIs for client-side and server-side tracking. Understanding when to use each is crucial for effective analytics implementation.
...
// ❌ Wrong - the root package exports shared types only
import { createClientAnalytics } from 'trakoo';
// ❌ Wrong - there is no provider aggregate export
...
<CardGroup>
  ...
  <Card title="Proxy Provider" href="/docs/providers/proxy" icon="arrow-right-left">
    Serverless-safe routing, batching, and server enrichment
  </Card>
</CardGroup>

...
await analytics.track('button_clicked', { buttonId: 'upgrade' });
...
Each call stands alone, so a server instance can safely handle many users. There's no persisted user state to reset.

## Type your traits

Pass a `typed<T>()` trait marker to type-check `identify()` alongside your events.
...
```typescript
async function handleUpgrade(user: User, plan: UserTraits['plan']) {

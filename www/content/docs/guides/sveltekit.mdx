...
```bash
npm install trakoo @trakoo/posthog posthog-js posthog-node
```

`@trakoo/posthog` contains the PostHog providers. PostHog ships separate browser and Node SDKs; this guide uses both. Other providers differ — see [Providers](/docs/providers) for each one's requirements.

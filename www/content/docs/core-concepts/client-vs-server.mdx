...
trackle provides separate APIs for client-side and server-side tracking. Understanding when to use each is crucial for effective analytics implementation.
...
import { createServerAnalytics } from 'trackle';

...
See [Installation](/docs/installation) for npm, yarn, bun, and the provider package matrix.
...
Install the PostHog provider package with PostHog's browser SDK:

```bash
pnpm add @trakoo/posthog posthog-js
```
...
The server provider uses PostHog's Node SDK:

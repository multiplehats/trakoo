...
Each event is attributed to its own caller, so one long-lived provider can serve concurrent requests. `context.device` is used as a fallback when `context.server` is absent. The IP travels as a header only — it is never stored as an event property, and a `device.ip` you pass is removed from the properties once it has been promoted.

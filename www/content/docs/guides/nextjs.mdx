...
This guide uses PostHog on both sides, so install the `@trakoo/posthog` provider package with both PostHog SDKs — one for the browser, one for Node:

```bash
npm install @trakoo/posthog posthog-js posthog-node
```

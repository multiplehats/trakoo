...
:::warning[Avoid Bento's built-in event names]
Because of the `$` prefix, a trakoo event named `subscribe`, `unsubscribe`, `tag`, `remove_tag`, `update_fields`, or `purchase` becomes one of Bento's built-in events and acts on the subscriber. For example, `unsubscribe` unsubscribes them. Give your events names that don't collide.
:::
...
<TypeTable
  type={{
    ...
    "clientOptions.baseUrl": {
      type: "string",
      default: "https://app.bentonow.com/api/v1",
      description: "Bento API base URL.",
    },
    logErrors: {
      type: "boolean",
      default: "false",
      description: "Make the Bento SDK log the HTTP response of failed requests.",
    },
    debug: {
      type: "boolean",
      default: "false",
      description: "Log provider activity to the console.",
    },
    enabled: {
      type: "boolean",
      default: "true",

...
Serverless functions terminate the moment they return, which can drop queued events. Call `shutdown()` before returning so providers can send everything they've buffered. This applies to server actions, route handlers, and the Edge runtime alike.
:::

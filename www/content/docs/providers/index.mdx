---
title: Providers Overview
description: Choose and configure analytics providers for your application
---

Providers are plugins that send your events to analytics services. trackle includes official support for popular services and makes it easy to add custom providers.

## Available Providers

<Cards>
  <Card title="PostHog" href="/docs/providers/posthog" icon="bar-chart">
    Open-source product analytics with feature flags and session replay
  </Card>
  <Card title="Bento" href="/docs/providers/bento" icon="mail">
    Email marketing and automation platform with event tracking
  </Card>
  <Card title="Pirsch" href="/docs/providers/pirsch" icon="shield">
    Privacy-focused, cookie-free web analytics
  </Card>
  <Card title="EmitKit" href="/docs/providers/emitkit" icon="bell">
    Event notifications and activity feeds with rich metadata
  </Card>
  <Card title="Visitors" href="/docs/providers/visitors" icon="eye">
    Lightweight analytics with Stripe revenue attribution
  </Card>
  <Card title="Proxy Provider" href="/docs/providers/proxy" icon="arrow-right-left">
    Route events through your server to bypass ad-blockers
  </Card>
</Cards>

## Quick Setup

### PostHog

```bash
pnpm install posthog-js posthog-node
```

```typescript
import { PostHogClientProvider } from 'trackle/providers/client';

new PostHogClientProvider({
  token: 'your-posthog-api-key',
  api_host: 'https://app.posthog.com'
})
```

[Full PostHog Documentation →](/docs/providers/posthog)

### Bento

```bash
# Client: No installation (uses CDN)
# Server: pnpm install @bentonow/bento-node-sdk
```

```typescript
import { BentoClientProvider } from 'trackle/providers/client';

new BentoClientProvider({
  siteUuid: 'your-bento-site-uuid'
})
```

[Full Bento Documentation →](/docs/providers/bento)

### Pirsch

```bash
pnpm install pirsch-sdk
```

```typescript
import { PirschClientProvider } from 'trackle/providers/client';

new PirschClientProvider({
  identificationCode: 'your-pirsch-identification-code',
  hostname: 'example.com'
})
```

[Full Pirsch Documentation →](/docs/providers/pirsch)

### EmitKit

```bash
# Server-only: pnpm install @emitkit/js
```

```typescript
import { EmitKitServerProvider } from 'trackle/providers/server';

new EmitKitServerProvider({
  apiKey: 'emitkit_xxxxx',
  channelName: 'analytics'
})
```

[Full EmitKit Documentation →](/docs/providers/emitkit)

### Visitors

```bash
# No installation needed (script loaded from CDN at runtime)
```

```typescript
import { VisitorsClientProvider } from 'trackle/providers/client';

new VisitorsClientProvider({
  token: process.env.NEXT_PUBLIC_VISITORS_TOKEN!
})
```

[Full Visitors Documentation →](/docs/providers/visitors)

## Using Multiple Providers

Combine providers to leverage different capabilities:

```typescript
import { createClientAnalytics } from 'trackle/client';
import {
  PostHogClientProvider,
  BentoClientProvider
} from 'trackle/providers/client';

const analytics = createClientAnalytics({
  providers: [
    // PostHog for product analytics and feature flags
    new PostHogClientProvider({
      token: import.meta.env.VITE_POSTHOG_KEY
    }),

    // Bento for email marketing automation
    new BentoClientProvider({
      siteUuid: import.meta.env.VITE_BENTO_SITE_UUID
    })
  ]
});

// Events sent to all providers
analytics.track('user_signed_up', {
  email: 'user@example.com',
  plan: 'pro'
});
```

## Environment-Specific Imports

Always use the correct import path to avoid bundling issues:

<Tabs items={['Client-Side', 'Server-Side']}>
<Tab value="Client-Side">

```typescript
// ✅ Correct - only browser-compatible code
import { PostHogClientProvider } from 'trackle/providers/client';

// ❌ Wrong - may bundle Node.js dependencies
import { PostHogClientProvider } from 'trackle/providers';
```

</Tab>
<Tab value="Server-Side">

```typescript
// ✅ Correct - only Node.js code
import { PostHogServerProvider } from 'trackle/providers/server';

// ❌ Wrong - may bundle browser code
import { PostHogServerProvider } from 'trackle/providers';
```

</Tab>
</Tabs>

## Configuration Best Practices

### 1. Use Environment Variables

```typescript
new PostHogClientProvider({
  token: import.meta.env.VITE_POSTHOG_KEY,  // Vite
  // OR
  token: process.env.NEXT_PUBLIC_POSTHOG_KEY  // Next.js
})
```

### 2. Enable Debug in Development

```typescript
new PostHogClientProvider({
  token: 'xxx',
  debug: import.meta.env.DEV  // or process.env.NODE_ENV === 'development'
})
```

### 3. Conditional Providers

```typescript
const providers = [
  new PostHogClientProvider({ token: 'xxx' })
];

// Only add Bento in production
if (import.meta.env.PROD) {
  providers.push(new BentoClientProvider({ siteUuid: 'xxx' }));
}

const analytics = createClientAnalytics({ providers });
```

### 4. Graceful Degradation

```typescript
const analytics = createClientAnalytics({
  providers: [
    new PostHogClientProvider({
      token: 'xxx',
      enabled: !!import.meta.env.VITE_POSTHOG_KEY  // Disable if no key
    })
  ]
});
```

## Custom Providers

Want to integrate with a different analytics service? Create a custom provider:

```typescript
import { BaseAnalyticsProvider, BaseEvent, EventContext } from 'trackle';

export class CustomProvider extends BaseAnalyticsProvider {
  name = 'CustomProvider';

  async initialize(): Promise<void> {
    // Initialize your SDK
  }

  track(event: BaseEvent, context?: EventContext): void {
    // Send event to your service
  }

  identify(userId: string, traits?: Record<string, unknown>): void {
    // Identify user
  }

  // ... implement other required methods
}
```

[Learn more about creating custom providers →](/docs/providers/custom)

## Next Steps

<Cards>
  <Card title="PostHog Provider" href="/docs/providers/posthog" icon="bar-chart">
    Product analytics with feature flags
  </Card>
  <Card title="Bento Provider" href="/docs/providers/bento" icon="mail">
    Email marketing and automation
  </Card>
  <Card title="Pirsch Provider" href="/docs/providers/pirsch" icon="shield">
    Privacy-focused web analytics
  </Card>
  <Card title="EmitKit Provider" href="/docs/providers/emitkit" icon="bell">
    Event notifications and activity feeds
  </Card>
  <Card title="Visitors Provider" href="/docs/providers/visitors" icon="eye">
    Lightweight analytics with Stripe revenue attribution
  </Card>
  <Card title="Custom Providers" href="/docs/providers/custom" icon="code">
    Create your own provider
  </Card>
</Cards>
